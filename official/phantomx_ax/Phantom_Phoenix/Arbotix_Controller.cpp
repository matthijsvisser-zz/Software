...
#define ARBOTIX_TO  3000        // if we don't get a valid message in this number of mills turn off
...
void CommanderInputController::ControlInput(void)
{
  ...
  if(command.ReadMsgs() > 0){
    ...
    if (command.buttons & BUT_L6 ) {
      ...
      if ((dspeed > 0) && (g_InControlState.SpeedControl < 3000)) {
        g_InControlState.SpeedControl += dspeed;
        if (g_InControlState.SpeedControl > 3000)
          g_InControlState.SpeedControl = 3000;
        MSound( 1, 50, 1000+g_InControlState.SpeedControl); 
      }
